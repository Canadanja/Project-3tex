...
\end{equation}
where $f(x)$ is a function evaluated at $x$ and $h$ is the step length for the derivation. This calculation is a very timeconsuming part of our computation. This is why later, in section~\ref{sec:closedform} and~\ref{sec:analytical}, we will consider the analytical expressions of the derivative.
The factor $J = \exp \left[ \frac{a_{12} r_{12}}{(1+\beta r_{12})} \right]$ is called the Jastrow factor, which we will refer to later.\\
...
\end{vmatrix}
\end{align}
Sure we want to calculate the local energy for six electron as well. For this purpose, we will generalize equation~\ref{glg:localenergy} as following for $N$ electrons:
\begin{equation}
...
\end{equation}
In this case, we refer to the particles positions $(\mathbf{r_1,r_2,r_3,r_4,r_5,r_6})=\mathbf{R}$.\todo{Slaterdeterminantenzerlegung erklären}
...
Furthermore, we generate random gaussian distributed random numbers using \texttt{gaussian}. \todo{different random numbers?}
